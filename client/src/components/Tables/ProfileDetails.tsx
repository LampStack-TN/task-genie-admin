    ...
    <div className="bg-white rounded-lg shadow-xl p-6 mx-auto max-w-4xl">
      ...
      <div className="mt-6 border-t border-gray-200">
        <dl className="divide-y divide-gray-200">
          ...
          <InfoItem label="Birthdate" value={professional?.birthdate || 'N/A'} />
          ...
          <InfoItem label="Verification Status" value={professional?.profile?.isVerified ? "Verified" : "Not Verified"} />
          {professional?.profile && (
            <>
              <DocumentItem label="Official Document" imageSrc={professional.profile.officialDocument} />
              <DocumentItem label="CIN Recto" imageSrc={professional.profile.cinRecto} />
              <DocumentItem label="CIN Verso" imageSrc={professional.profile.cinVerso} />
            </>
          )}
        </dl>
      </div>
...
const InfoItem = ({ label, value }: { label: string, value: string | undefined }) => (
    <div className="py-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
        ...
        <dd className="mt-1 text-sm text-gray-900 sm:col-span-2">{value || 'N/A'}</dd>
    </div>
);

const DocumentItem = ({ label, imageSrc }: { label: string, imageSrc?: string }) => (
    <div className="py-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
        <dt className="text-sm font-medium text-gray-500">{label}</dt>
        <dd className="mt-1 text-sm text-gray-900 sm:col-span-2">
            {imageSrc ? <img src={imageSrc} alt={`${label}`} className="max-w-xs rounded-lg shadow-sm" /> : 'No Image Available'}
        </dd>
    </div>
);
